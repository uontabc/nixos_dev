      ...
      boot.initrd.systemd.services.impermanence-rollback = {
        ...
        script = ''
          ...
          if [ -d /btrfs-tl/@root-blank ]; then
            ...
            # -R: root contains nested subvolumes (systemd-tmpfiles creates
            # /tmp, /var/tmp, /srv, /var/lib/machines, /var/lib/portables as
            # btrfs subvolumes). A plain delete fails with ENOTEMPTY, so every
            # rollback silently failed since this machine was set up.
