{
  ...
  # of truth, shared with the diskoConfigurations.laptop flake output). The
  # file is underscore-prefixed so import-tree does NOT import it as a
  # flake-parts module (`disko.*` is not a flake-parts option).
  flake.modules.nixos.laptop = ./_disko-devices.nix;
}
